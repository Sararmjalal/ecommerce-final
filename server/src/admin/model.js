...
class AdminSchema {
  ...
  async create(data) {
    ...
    if (doesExist)
      throw new Error(
        "bad request: this phonenumber already exists in the database"
      );
    ...
    writeFileSync(
      path.join(dbDirectory, `${thisAdmin._id}.txt`),
      JSON.stringify(thisAdmin),
      "utf8"
    );
  ...
  async generateAuthObject(phone) {
    ...
    const thisAdmin = await this.findByPhone(validPhone);
  ...
  async checkAuthCode({ phone, code }) {
    ...
    if (
      new Date().getTime() - new Date(thisAdmin.authObj.date).getTime() >
      200000
    )
