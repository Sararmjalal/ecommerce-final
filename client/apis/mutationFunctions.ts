...
const editProduct = async (productId: string, data: ProductBody) =>
...
const userSignupOne = async (phone: string, name: string) =>
  await postRequest("/user/sign-up-one", {phone, name});

const userSignupTwo = async (phone: string, code: string) =>
  await postRequest("/user/sign-up-two", {phone, code});

const userLoginOne = async (phone: string) =>
  await postRequest("/user/login-one", {phone});

const userLoginTwo = async (phone: string, code: string) =>
  await postRequest("/user/login-two", {phone, code});

const userInfo = async () => await postRequest("/user/me", {}, "user");

const createAddress = async (location: Location, receiver: Receiver) =>
  await postRequest("/address/create", {location, receiver}, "user");

const editAddress = async (addressId: string, data: AddressData) =>
...
const submitComment = async (productId: string, text: string) =>
