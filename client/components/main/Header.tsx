  ...
  return (
    <>
      <div
        ...
        className={`absolute ${thisAdmin ? 'top-8' : "top-0"} py-8 xxl:px-16 xl:px-14 lg:px-6 px-[160px] grid grid-cols-3 lg:grid-cols-2 sm:flex sm:justify-between sm:items-center items-start w-full text-white lg:text-black z-[1000]`}>
        <Link href={"/"}>
        ...
        <div className='lg:hidden flex justify-center items-center gap-20'>
          <Link href={"/"}
            onClick={() => setHeaderHandler({ ...headerHandler, openMenu: false })}>
          ...
          <p
            ...
            onMouseEnter={() =>
              categories[0] &&
              setHeaderHandler({ ...headerHandler, openMenu: true })}
            onClick={() => categories[0] ? 
              setHeaderHandler({ ...headerHandler, openMenu: !headerHandler.openMenu })
              :
          ...
          {thisUser ? (
            <div className='relative flex gap-9 lg:gap-7'>
              <AiOutlineUser
                ...
                onClick={() => setHeaderHandler({ ...headerHandler, openUserMenu: !headerHandler.openUserMenu })}
              />
              <AiOutlineLogout
                cursor={"pointer"}
                className='lg:hidden xs:text-lg text-2xl'
                onClick={() => setHeaderHandler({ ...headerHandler, openConfirm: !headerHandler.openConfirm })}
              />
              {headerHandler.openUserMenu && (
                <UserMenu
                  userMenu={userMenu}
                  closeHandler={() => setHeaderHandler({ ...headerHandler, openUserMenu: !headerHandler.openUserMenu })}
                  confirmHandler={() => setHeaderHandler({...headerHandler, openConfirm: true, openUserMenu: false })}
                />
              )}
            </div>
          ) : (
            <AiOutlineLogin
              cursor={"pointer"}
              className='xs:text-lg text-2xl'
              onClick={() => setHeaderHandler({ ...headerHandler, openLogin: !headerHandler.openLogin })}
            />
          )}
          <AiOutlineMenu
            cursor={"pointer"}
            className='hidden lg:block xs:text-lg text-2xl'
            onClick={() => categories[0] ? setHeaderHandler({ ...headerHandler, openMenu: !headerHandler.openMenu })
              :
            toast.info("No products available yet.")}
          />
        </div>
      ...
      <Login
        ...
        closeHandler={() => {setHeaderHandler({ ...headerHandler, mode: "login", openLogin: false })}}
      ...
      {headerHandler.openMenu &&
        <Menu
          closeHandler={() => setHeaderHandler({ ...headerHandler, openMenu: false })}
          categories={categories}
        />
      }
      { headerHandler.openCart &&
        <CartModal
        closeHandler={() => setHeaderHandler({ ...headerHandler, openCart: false })}
        />
      }
      {headerHandler.openConfirm && (
        <ConfirmModal
          mode='logout'
          closeHandler={() => setHeaderHandler({ ...headerHandler, openConfirm: !headerHandler.openConfirm })}
          okHandler={() => {
